...
@Component
public class RecipientEmailLengthEmailOciImpl implements EmailValidationStrategy {
    @Override
    public void execute(@Valid EmailDTO emailDTO) {
        if(emailDTO.recipient().length() > 40) {
            throw new EmailServiceException("Email destinatário deve ter no máximo 40 caracteres");
        }
    }
