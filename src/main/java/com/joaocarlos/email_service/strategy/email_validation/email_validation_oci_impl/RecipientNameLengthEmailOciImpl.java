...
@Component
public class RecipientNameLengthEmailOciImpl implements EmailValidationStrategy {
    @Override
    public void execute(@Valid EmailDTO emailDTO) {
        if (emailDTO.recipientName().length() > 50) {
            throw new EmailServiceException("Nome destinatário deve ter no máximo 50 caracteres");
        }
    }
