...
import com.joaocarlos.email_service.strategy.email_service.impl.EmailServiceAwsImpl;
...
public class EmailServiceAwsImplTest {
    @InjectMocks
    private EmailServiceAwsImpl emailAwsImpl;
    @Mock
    ...
    void getNameServiceEmail_ReturnsNameServiceEmail() {
        assertEquals("AWS", emailAwsImpl.getNameServiceEmail());
    }

    @Test
    void sendEmail_WithEmailDTONull_ThrowsException() {
        assertThrowsExactly(IllegalArgumentException.class, () -> emailAwsImpl.sendEmail(null));
    }

    @Test
    void sendEmail_WithValidEmailDTO_DoesNotThrowException() {
        when(emailValidationFactory.getEmailValidationStrategy("AWS")).thenReturn(List.of(emailValidationStrategy));
